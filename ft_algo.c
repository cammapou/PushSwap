...
void ft_quick_b1(t_ev *ev, t_list **lst_a, t_list **lst_b)
{
	...
	med = ft_check_med0(lst_b);
    printf("med = %d\n", med);
    ev->len_b = ft_stack_len_b(*lst_b);
    printf("med = %d\n", ev->len_b);
	while (ev->len_b)
	{
        while (ft_stack_de(*lst_b) == 0)
        {
...
void ft_quick_b5(t_ev *ev, t_list **lst_a, t_list **lst_b)
{
	int med;
	int len_b;
	int med2;
